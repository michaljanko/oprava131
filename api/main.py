...
# Determine the path to the dataset relative to this file. When deployed on Vercel
# we include data.json inside the same folder (api/) so that the function can access it.
...
try:
    ...
    # Fall back to reading data.json from the parent directory if present
    fallback_path = Path(__file__).resolve().parent.parent / "data.json"
